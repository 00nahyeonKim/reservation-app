# 스터디룸 예약 시스템 (Roomy) 프로젝트 보고서

---

## 요약

**프로젝트명:** Roomy — 실무형 동시성 제어 스터디룸 예약 시스템  
**개발 기간:** 2026.05.21 ~ 2026.05.29  
**팀 구성:** 2인 (백엔드 담당: 김나현 / 프론트엔드 담당: 팀원)  
**목적:** 동시에 같은 시간대에 예약 요청이 몰릴 경우 단 1건만 성공하도록 비관적 락(Pessimistic Lock)으로 데이터 무결성을 보장하는 예약 시스템 구현
...
| Database | Oracle XE (Docker), Spring Data JPA |
| 인증 | Spring Security, 서버 세션(JSESSIONID) |
| Frontend | React 19, Vite, Axios (팀원 담당) |
...
| E-02 | 동일 회의실, 동일 날짜, 동일 시작 시간의 중복 예약은 불가하다 | DB 복합 유니크 제약 |
| E-03 | 사용자는 자신의 예약 목록을 조회할 수 있다 | |
| E-04 | 사용자는 자신의 예약만 취소할 수 있다 | Soft Delete (상태: CANCELED) |
| E-05 | 날짜와 회의실을 조건으로 예약 현황을 조회할 수 있다 | 로그인 불필요 |
...
| 데이터 무결성 | DB 레벨 복합 유니크 제약 + 애플리케이션 레벨 중복 검증 이중 방어 |
...
| 상태코드 | 상황 |
|----------|------|
| 400 | 입력값 검증 실패 (아이디/비밀번호 형식 오류 등) |
...
| 로그인 API — Controller 세션 처리 | 진행 중 |

#### 3.4 추후 진행 예정

| 예정 항목 | 내용 |
|-----------|------|
| 로그인 완료 | Controller 세션 바인딩 마무리 + BCrypt 패스워드 암호화 적용 |
| 사용자 API 완성 | 로그아웃, 내 정보 조회 구현 |
| 회의실 API | POST /api/rooms, GET /api/rooms, GET /api/rooms/{id} |
| 예약 API 전체 | 예약 생성·취소·조회 서비스 및 컨트롤러 구현 |
| 비관적 락 적용 | ReservationRepository에 `@Lock(PESSIMISTIC_WRITE)` 선언, Oracle `SELECT FOR UPDATE` 동작 확인 |
