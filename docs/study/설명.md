...
### entity/Reservation.java (수정 2곳)

**① `@UniqueConstraint` 제거**

`@Table` 어노테이션에서 `uniqueConstraints` 속성 전체를 삭제한다.
overlap은 서비스 레이어에서 관리하므로 DB 제약이 불필요하다.
...
```java
// 기존 endTime 필드 아래에 추가
@Column(name = "head_count", nullable = false)
private Integer headCount;
```

`@Builder` 생성자에도 `int headCount` 파라미터와 `this.headCount = headCount;` 대입 추가.
...
public class ReservationService {
    ...
    public ReservationResponse create(Long userId, ReservationCreateRequest request) {
        ...
        // 예약 시간 유효성 검증 (30분 이상, 4시간 이하)
        validateReservationTime(request.getStartTime(), request.getEndTime());

        // 최소 예약 인원 검증
        int minHeadCount = getMinHeadCount(room.getCapacity());
        if (request.getHeadCount() < minHeadCount) {
    ...
    // 예약 시간이 30분 이상 4시간 이하인지 검증한다.
    private void validateReservationTime(String startTime, String endTime) {
    ...
    // capacity 값에 따라 최소 예약 인원을 반환한다.
    // 5인실(A·B·C): 3명, 10인실(I·J): 6명, 나머지(6·8인실): 4명
    private int getMinHeadCount(int capacity) {
